...
| Teinte sémantique secondaire par page | déclarée dans le catalogue de la page, une famille parmi `macro`, `option`, `warning` (source typée `pageAccent`, `apps/web/src/design/tokens.ts`) ; exposée par `[data-page-accent="<famille>"]` → `--vx-page-accent`, `--vx-page-accent-soft`, `--vx-page-accent-gradient-start/-end` | attribut de composition | famille hors vocabulaire → erreur de type ; `signal` (ambre), `positive` et `negative` ne sont pas éligibles |

Règles qui accompagnent ces formes :

- **Une famille par page.** La teinte secondaire garde le sens de sa famille (`macro` = contexte : série contextuelle ou catégorisation de source, jamais un verdict ; `option` = domaine options ; `warning` = prudence, retard, synthétique). `positive` et `negative` restent réservés au signe financier servi (`charts.md`, « Réserver vert/rouge au signe financier » ; `VERTEX_ONE_VISUAL_DIRECTION.md`, « restent exclusivement financiers ») : une teinte de page ne bascule pas selon le signe, elle ne peut donc pas en porter un — un anneau, un arc ou une aire verts sur une valeur sans signe seraient un faux rassurant (cf. `apps/web/src/pages/risk/riskModules.ts`, « une jauge verte sur des risques non mesurés »). Elle n'est jamais décorative, jamais choisie par un hex libre, et l'ambre de marque reste la seule lumière de la dominante (tranche, kicker, sélection).
