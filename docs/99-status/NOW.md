...
completed:
  ...
  - research: walk-forward purgé (la purge est prouvée non triviale), calibration
      Brier/log-loss/fiabilité/bootstrap en blocs, RÈGLE D'ABSTENTION, frontière
      testée interdisant tout import de runtime — aucun modèle, aucune donnée
checks_locaux:
  - "pytest 2298 passed / 0 failed (dont research/ et tools/)"
  - "intégration PostgreSQL : 96 + 17 + 53 passed (SÉRIELLE obligatoire — base partagée)"
  - "vitest 254 passed ; tsc 0 erreur ; build OK"
  - "playwright 234 passed (3 viewports + smoke 1024, pipeline reel worker+API+web)"
