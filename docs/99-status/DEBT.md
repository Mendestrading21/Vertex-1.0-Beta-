...
| Budget de fraîcheur au relais — ASYMÉTRIE SYSTÉMIQUE | Vérifié par recherche sur les 8 relais : seuls `calendar.py` et `opportunities.py` recalculent la fraîcheur contre l'horloge du relais. `analysis`, `markets`, `options`, `performance`, `follow_up` et `portfolio` relaient l'état publié par le worker **sans jamais le réévaluer**. Conséquence : si le worker s'arrête, un snapshot publié `ok` reste servi `ok` indéfiniment. C'est exactement le défaut P1-E, corrigé sur deux relais et **ouvert sur six**. À traiter comme une vague dédiée, pas comme un correctif ponctuel. |
...
| Contrat de valeur des relais | Les **10 relais** appliquent désormais `checked_relayed_content` : avec le payload du 4e audit (5038 caractères, BEL + ANSI), **0 %** de champ chaîne est relayé verbatim, contre 57 % à 100 % avant. **Résidu chiffré, non fermé** : avec une valeur *anodine* de 18 caractères, 5 % à 30 % des champs passent encore selon le relais — exclusivement des champs de PROSE (`title`, `reason`, `method`, `detail`, `hypotheses`…). Ils sont bornés en longueur et refusent les caractères de contrôle ; aucun vocabulaire ne leur est imposé parce qu'il n'en existe aucun. Un `title` de 400 caractères disant « gain garanti » passe donc toujours. Aucun échappement HTML/JS n'est appliqué : la défense d'affichage appartient à `apps/web` et n'a pas été vérifiée. |
| Détecteur de langage d'ordre — couverture MESURÉE | Le 4e audit l'a chiffré sur un corpus adversarial de 72 formulations : **49 %** (35/72), avec **0 faux positif** sur 8 libellés financiers légitimes. Non détectés : italien, néerlandais, polonais, turc, japonais, translittération russe, et des idiomes anglais (« Load up on this », « Scale in »), ainsi que des formulations de certitude ou de probabilité non listées. La seule garantie qui tient est STRUCTURELLE (jamais dans une claim FACT, cantonné en `EXTERNAL_UNVERIFIED`, échappé, borné, limitation visible), pas lexicale. |
